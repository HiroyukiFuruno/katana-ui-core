...
#[test]
fn spacer_inspector_options_mutate_layout_gap_semantics_and_body() -> Result<(), String> {
...
fn expected_states() -> &'static [(&'static str, &'static str)] {
    &[
        ("variant", "spacer.variant=alternate"),
        ("tone", "spacer.tone=accent"),
        ("size", "spacer.size=large"),
        ("theme.slot", "spacer.theme.slot=custom"),
    ]
}
...
fn option_index(setting: &str) -> Result<usize, String> {
    storybook_ui_option_contract::options_for_page(PAGE)
        ...
        .ok_or_else(|| format!("missing spacer option `{setting}`"))
}
