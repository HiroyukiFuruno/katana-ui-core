...
fn expected_states() -> &'static [(&'static str, &'static str)] {
    &[
        ...
        (
            "virtualization.focused_index",
            "virtualization.focused_index=42",
        ),
        (
            "virtualization.measured_correction",
            "virtualization.measured_correction=+8",
        ),
        (
            "virtualization.row_height_provider",
            "virtualization.row_height=variable",
        ),
    ]
}
...
fn option_index(setting: &str) -> Result<usize, String> {
    storybook_ui_option_contract::options_for_page(PAGE)
        ...
        .ok_or_else(|| format!("missing virtualization option `{setting}`"))
}
