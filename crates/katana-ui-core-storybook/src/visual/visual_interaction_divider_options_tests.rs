...
#[test]
fn divider_inspector_options_mutate_separator_semantics_and_body() -> Result<(), String> {
...
fn expected_states() -> &'static [(&'static str, &'static str)] {
    &[
        ("variant", "divider.variant=alternate"),
        ("tone", "divider.tone=accent"),
        ("size", "divider.size=large"),
        ("theme.slot", "divider.theme.slot=custom"),
    ]
}
...
fn option_index(setting: &str) -> Result<usize, String> {
    storybook_ui_option_contract::options_for_page(PAGE)
        ...
        .ok_or_else(|| format!("missing divider option `{setting}`"))
}
