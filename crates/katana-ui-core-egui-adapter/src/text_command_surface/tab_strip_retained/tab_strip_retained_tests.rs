...
fn tab_strip_retained_error_display_covers_all_variants() {
    ...
    let raster_error = TabStripRetainedError::Raster(
    ...
    let svg_error = TabStripRetainedError::Svg(
        katana_ui_core_svg_raster::UiSvgRasterError::InvalidDimensions {
            width_px: 0,
            height_px: 0,
        },
    );
    ...
    let surface_error = TabStripRetainedError::TextSurface(
        crate::text_surface::EguiTextSurfaceError::FrameNotProduced,
    );
    assert!(
        surface_error
            .to_string()
