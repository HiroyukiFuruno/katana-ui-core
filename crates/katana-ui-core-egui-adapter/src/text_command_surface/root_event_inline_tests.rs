...
fn router_context_snapshots_all_generic_payloads_without_consuming_root_batch() {
    let search_events = vec![
        CommandChromeSearchEvent::Strip {
            event:
                katana_ui_core::molecule::structured::SearchControlStripEvent::SearchQueryChanged(
                    String::from("needle"),
                ),
        },
        CommandChromeSearchEvent::Strip {
            event:
                katana_ui_core::molecule::structured::SearchControlStripEvent::ReplaceValueChanged(
                    String::from("replacement"),
                ),
        },
        CommandChromeSearchEvent::Strip {
            event:
                katana_ui_core::molecule::structured::SearchControlStripEvent::SearchOptionChanged {
...
struct StageFailingDispatcher {
    fail_at: EguiTextCommandSurfaceRootEventChildClass,
    calls: Vec<EguiTextCommandSurfaceRootEventChildClass>,
}
...
fn command_detach_clears_text_only_when_command_activation_is_present() {
    let payload = RootEventPayload {
        text: vec![TextSurfaceEvent::TextArea(
            katana_ui_core::atom::TextAreaEvent::Change("activated".into()),
        )],
        toolbar: Some(vec![CommandChromeToolbarEvent::CommandActivated {
