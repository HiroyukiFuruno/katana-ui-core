...
mod coverage_tests {
    use super::*;
    use super::super::super::sanitized_command_projection::SanitizedCommandCapabilityRejection;
    use super::super::super::sanitized_context_projection::SanitizedContextMenuCapabilityRejection;
    use super::super::super::sanitized_document_root_transport::{
        SanitizedDocumentRootEventDispatchError, SanitizedDocumentRootEventForwardError,
    };
    use super::super::super::sanitized_search_projection::{
        ...
        SanitizedSearchTextOperation, SanitizedSearchTextPresentation,
        SanitizedSearchUnavailablePresentation, SanitizedSearchUnitOperation,
    };
    use super::super::super::sanitized_tab_projection::SanitizedTabGroupTarget;
    use super::super::{
    ...
    impl SanitizedDocumentRootEventForwarder for RecordingForwarder {
        ...
        fn forward_sanitized_document_root_event(
            ...
            transport
                .dispatch_root_once(&mut TestRootDispatcher)
                .map_err(|_| ())?;
            Ok(())
        }
    }
    ...
    impl RetainingForwarder {
        ...
        > {
            if let Some(transport) = self.transport.as_mut() {
                transport.dispatch_root_once(&mut TestRootDispatcher)
            } else {
    ...
    fn input_with_search(revision: u64) -> SanitizedDocumentRootInput {
        input(revision, b"document", "本文 ⭐️").with_search_projection(
            super::super::super::sanitized_document_root_process::search_projection_for_ime(
    ...
    fn physical_raw_input_routes_text_replace_and_navigation_operations() {
        ...
        let (output, _) = run_search_root_frame(&context, &mut root, Vec::new());
        let query =
            accesskit_bounds(&output, egui::accesskit::Role::TextInput, "検索語 ⭐️").center();
    ...
    fn readonly_raw_text_and_ime_do_not_mutate_but_pointer_selection_remains_available() {
        ...
        let end = egui::pos2(content.x as f32 + content.width as f32 - FLOATING_SURFACE_HORIZONTAL_OFFSET, start.y);
        ...
        for events in [
            ...
            vec![egui::Event::Ime(egui::ImeEvent::Preedit { text: "下書き ⭐️".to_string(), active_range_chars: None })],
            vec![egui::Event::Ime(egui::ImeEvent::Commit(
                "確定 ⭐️".to_string(),
            ))],
        ] {
    ...
    fn readonly_does_not_assign_semantics_to_an_enabled_opaque_command() {
        ...
        let projection = super::super::super::SanitizedCommandProjection::new([
            super::super::super::SanitizedCommandGroup::new(1, "generic").item(
                super::super::super::SanitizedCommandItem::new(
                    super::super::super::SanitizedCommandTarget::from_opaque_bytes([7])
        ...
        let (_, released) =
            run_command_root_frame(&context, &mut root, vec![accesskit_click(node)]);
        assert_eq!(
            released
                .command_events
                .borrow()
    ...
    fn raw_ime_search_at_current_root_renders_and_forwards_one_opaque_event() {
        ...
        let (initial_output, initial_frame) =
            run_search_root_frame(&context, &mut root, Vec::new());
        ...
        let (_, preedit) = run_search_root_frame(
            &context,
            &mut root,
            vec![egui::Event::Ime(egui::ImeEvent::Preedit { text: "かな".to_string(), active_range_chars: None })],
        );
    ...
    fn command_keyboard_activation_uses_root_raw_input_and_one_shot_transport() {
        ...
        let (_, frame) =
            run_command_root_frame(&context, &mut root, vec![key_press(egui::Key::Enter)]);
        let mut forwarder = RecordingForwarder {
            calls: 0,
            transport_debug: None,
        };
    ...
    fn select_floating_surface(
        ...
        let start = egui::pos2(
            content_bounds.x as f32 + FLOATING_SURFACE_HORIZONTAL_OFFSET,
        ...
        let end = egui::pos2(
            content_bounds.x as f32 + content_bounds.width as f32 - FLOATING_SURFACE_HORIZONTAL_OFFSET,
            start.y,
        );
    ...
    fn physical_context_pointer_nested_keyboard_and_accesskit_are_one_shot() {
        for route in ["pointer", "nested", "keyboard", "accesskit"] {
            ...
            let _ =
                run_root_frame_events(&context, &mut root, vec![egui::Event::PointerMoved(point)]);
            ...
            let frame = if route == "nested" {
                ...
                let _ =
                    run_root_frame_events(&context, &mut root, vec![egui::Event::PointerMoved(p)]);
                ...
                let _ =
                    run_root_frame_events(&context, &mut root, vec![egui::Event::PointerMoved(cp)]);
    ...
    fn physical_text_selection_controls_floating_surface_lifecycle() {
        ...
        let top_projection = SanitizedCommandProjection::new([SanitizedCommandGroup::new(
            1,
            "トップ操作 日本語 ⭐️",
        )
        ...
        let escaped =
            run_root_frame_events(&context, &mut root, vec![key_press(egui::Key::Escape)]).1;
        assert!(
            escaped
                .output
                .floating
        ...
        assert!((outside.x.round() as i32) < floating.panel_bounds.x);
        assert!((outside.y.round() as i32) < floating.panel_bounds.y);
    ...
    fn physical_floating_keyboard_activation_routes_only_to_floating_target_once() {
        ...
        input =
            input.with_floating_command_projection(SanitizedCommandProjection::new([
                SanitizedCommandGroup::new(0, "浮遊操作 日本語 ⭐️").item(
                    SanitizedCommandItem::new(floating_target, 0, "太字 日本語 ⭐️"),
                ),
            ]));
        ...
        for _ in 0..8 {
            ...
            let candidate =
                run_root_frame_events(&context, &mut root, vec![key_press(egui::Key::ArrowRight)])
                    .1;
            assert_eq!(
                candidate
                    .command_events
    ...
    fn physical_floating_accesskit_snapshot_click_routes_one_opaque_target() {
        ...
        let (node, _) = output
            ...
            .find_map(|(node_id, node)| {
                ...
                let matches_action = node.role() == egui::accesskit::Role::Button
                    ...
                    && bounds.x1 as i32
                        == action_bounds.x.saturating_add_unsigned(action_bounds.width)
                    && bounds.y1 as i32
                        == action_bounds
                            .y
        ...
        let (_, activated) =
            run_root_frame_events(&context, &mut root, vec![accesskit_click(node)]);
        assert_eq!(activated.output.events().event_cardinality(), 0);
        assert_eq!(
            activated
                .command_events
    ...
    fn physical_floating_failure_matrix_is_strict_and_stale_safe() {
        ...
        for (name, enabled, visible, capability, reject) in cases {
            ...
            if name == "hidden" {
                ...
                let _ =
                    run_root_frame_events(&context, &mut root, vec![pointer_button(point, true)]);
        ...
        let (_, rejection_selected) =
            select_floating_surface(&rejection_context, &mut rejection_root);
    ...
    fn context_menu_missing_and_callback_rejection_fail_at_root_frame() {
        for (capability, reject, expected) in [
            ...
            let _ =
                run_root_frame_events(&context, &mut root, vec![pointer_button(item_point, true)]);
            if !capability {
                let result = run_root_frame_result(
                    &context,
                    &mut root,
            ...
            let frame = run_root_frame_result(
                &context,
                &mut root,
                vec![pointer_button(item_point, false)],
            )
