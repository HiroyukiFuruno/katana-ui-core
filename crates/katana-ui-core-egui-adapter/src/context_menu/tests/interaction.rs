...
fn context_menu_local_anchor_returns_node_id_without_translation() {
    ...
    let anchor = TextSurfaceContextTargetAnchor {
        anchor: katana_ui_core::render_model::UiContextMenuAnchor::NodeId("menu-target".to_owned()),
        selection: UiTextSelectionRange::caret(0),
        viewport_bounds: viewport,
    };
    assert_eq!(
        katana_ui_core::render_model::UiContextMenuAnchor::NodeId("menu-target".to_owned()),
...
fn context_menu_paint_plan_captures_icon_texture_branch() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert!(operations.iter().any(|operation| matches!(&operation.kind, crate::context_menu::types::ContextMenuPaintOperationKind::Texture { texture, .. } if texture.identity.starts_with("context-menu-icon:"))));
    Ok(())
}
