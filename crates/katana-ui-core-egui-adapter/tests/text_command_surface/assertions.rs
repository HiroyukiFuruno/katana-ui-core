    ...
    for (position, (child, plan)) in output.artifact_order().iter().zip(plans.iter()).enumerate() {
        match (child, plan) {
            ...
            (
                ...
                panic!("SourceAddress layer resolved to a command-chrome plan")
            }
            ...
            (other_child, ArtifactPaintPlanRef::CommandChrome(_)) => {
                panic!(
                    "artifact plan at position {position} is CommandChrome but child is {other_child:?}"
                );
            }
            ...
            (other_child, ArtifactPaintPlanRef::SourceAddress(_)) => {
                panic!(
                    "artifact plan at position {position} is SourceAddress but child is {other_child:?}"
                );
            }
            ...
            (other_child, ArtifactPaintPlanRef::TabStrip(_)) => {
                panic!(
                    "artifact plan at position {position} is TabStrip but child is {other_child:?}"
                );
            }
            (other_child, ArtifactPaintPlanRef::StatusBar(_)) => {
                panic!(
                    "artifact plan at position {position} is StatusBar but child is {other_child:?}"
                );
            }
            (other_child, ArtifactPaintPlanRef::DiagnosticsList(_)) => {
                panic!(
                    "artifact plan at position {position} is DiagnosticsList but child is {other_child:?}"
                );
            }
        }
    }
