...
mod tests {
    ...
    fn frame_presentation_uses_public_associated_function() {
        ...
        let raster_host_mod_source = include_str!("mod.rs");

        assert!(!lib_source.contains("present_frame,"));
        assert!(!raster_host_mod_source.contains("present_frame,"));
        assert!(lib_source.contains("pub mod raster_host"));
        assert!(raster_host_mod_source.contains("StorybookPresentation"));
    }
