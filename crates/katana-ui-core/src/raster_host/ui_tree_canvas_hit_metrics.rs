...
mod tests {
    ...
    #[test]
    fn metrics_cover_absolute_row_column_image_and_scroll_boundaries() {
        use katana_ui_core::render_model::{
            UiCommonProps, UiEdgeInsets, UiImageSurfaceProps, UiPosition,
        };

        let absolute = UiNode::new(UiNodeKind::Button, "absolute")
            .width(UiDimension::Px(30))
            .height(UiDimension::Px(10))
            .margin(UiEdgeInsets {
                top: UiDimension::Px(2),
                right: UiDimension::Px(5),
                bottom: UiDimension::Px(7),
                left: UiDimension::Px(3),
            })
            .position(UiPosition::Absolute);
        let rect = absolute_child_rect(10, 20, 100, 50, &absolute);
        assert_eq!(75, rect.x);
        assert_eq!(53, rect.y);

        let row = UiNode::new(UiNodeKind::Row, "")
            .common(UiCommonProps::default().gap(UiDimension::Px(3)))
            .child(
                UiNode::new(UiNodeKind::Button, "first")
                    .width(UiDimension::Px(10))
                    .height(UiDimension::Px(11)),
            )
            .child(UiNode::new(UiNodeKind::Checkbox, "second"));
        assert_eq!(41, control_row_width(&row, 1));
        assert_eq!(TEXT_HEIGHT, control_row_height(&row, 1));

        let column = UiNode::new(UiNodeKind::Column, "")
            .common(UiCommonProps::default().gap(UiDimension::Px(2)))
            .child(row.clone())
            .child(UiNode::new(UiNodeKind::Text, "wide"));
        assert_eq!(56, control_row_width(&column, 1));
        assert!(control_row_height(&column, 1) > TEXT_HEIGHT);

        let parent = UiNode::new(UiNodeKind::Stack, "").child(absolute.clone());
        assert!(has_absolute_child(&parent));
        assert!(is_absolute(&absolute));
        assert_eq!(3, row_indent(&absolute));
        assert!(should_draw_container_label(&UiNode::new(
            UiNodeKind::Card,
            "Card"
        )));
        assert!(!should_draw_container_label(&UiNode::new(
        ...
        let image = UiImageSurfaceProps::new("image", 4, 2, vec![0xff; 4 * 2 * 4])
            .expect("test image must be valid");
        let image_node = UiNode::new(UiNodeKind::ImageSurface, "").image_surface(image);
        assert_eq!(2, child_height(&image_node));
        assert_eq!(0, dimension_px(&UiDimension::Percent(50)));
        assert_eq!(
            1,
            remaining_width(
                UiTreeRenderArea {
        ...
        assert_eq!(
            5,
            render_origin_y(&UiNode::new(UiNodeKind::Stack, ""), area)
        );
        assert_eq!(
            9,
            render_origin_y(&UiNode::new(UiNodeKind::ScrollArea, ""), area)
        );
    }
}
