...
#[test]
fn surface_host_action_hits_keep_fractional_accordion_headers_aligned() {
    let document_typography = UiTreeDocumentTypography::new()
        .with_body_baseline(UiTreeTextRoleBaselineTypography::new(16.0, 31.5, 18.5));
    let accordion = |action_id: &str| {
        UiNode::new(UiNodeKind::Accordion, action_id)
            .text(UiTextProps {
                role: "html-accordion".to_owned(),
                ..UiTextProps::default()
            })
            .host_action(UiHostActionSpec::command(action_id, action_id))
    };
    let root = UiNode::new(UiNodeKind::Column, "")
        .child(accordion("first"))
        .child(accordion("trailing"));
    let host =
        UiTreeSurfaceHost::with_document_typography(ThemeSnapshot::dark(), document_typography);
    ...
    let trailing = hits
        .iter()
        .find(|hit| hit.action.action_id == "trailing")
        .kuc_expect("trailing accordion action hit");

    assert_eq!(31, trailing.rect.y);
}

#[test]
fn surface_host_action_hits_keep_fractional_scroll_offsets_aligned() {
    let document_typography = UiTreeDocumentTypography::new()
        .with_body_baseline(UiTreeTextRoleBaselineTypography::new(16.0, 31.5, 18.5));
    let first: UiNode = Text::new("First").text_role("body").into();
    let trailing: UiNode = Text::new("Trailing")
    ...
    let hits = host.document_host_action_hits(&root, area);
    let trailing = hits
        .iter()
        .find(|hit| hit.action.action_id == "trailing")
        .kuc_expect("trailing scroll action hit");
