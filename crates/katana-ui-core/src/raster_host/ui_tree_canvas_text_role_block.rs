...
mod tests {
    ...
    use katana_ui_core::render_model::{
        UiBorder, UiCommonProps, UiDimension, UiEdgeInsets, UiInteractionState, UiNodeKind,
        UiTextProps,
    };
    ...
    #[test]
    fn heading_code_and_quote_cover_border_depth_padding_and_hover_variants() {
        let palette = palette();
        let mut canvas = Canvas::new(WIDTH, HEIGHT, palette.background);
        let heading = UiNode::new(UiNodeKind::Text, "heading").text(UiTextProps {
            role: "heading".to_owned(),
            ..UiTextProps::default()
        });
        let heading_metrics = UiTreeTextMetrics::for_node(&heading);
        draw_heading(
            &mut canvas,
            &heading,
            4,
        ...
        let heading = heading.border(UiBorder::solid(1, 0, "heading.border"));
        draw_heading(
            &mut canvas,
            &heading,
            4,
            ...
            area(),
            palette,
            UiTreeTextMetrics::for_node(&heading),
        );
        assert!(canvas.pixels().contains(&palette.selection));

        for common in [
            UiCommonProps::default(),
            UiCommonProps::default().border(UiBorder::solid(1, 0, "code.border")),
            UiCommonProps::default()
                .border(UiBorder::solid(1, 0, "code.border"))
                .margin(UiEdgeInsets {
                    left: UiDimension::Px(32),
            ...
            draw_code(
                ...
                area(),
                palette,
                UiTreeTextMetrics::for_node(&code),
            );
        }
        assert!(canvas.pixels().contains(&palette.code_background));
        assert!(canvas.pixels().contains(&palette.muted_border));

        let quote = UiNode::new(UiNodeKind::Text, "quote")
            .text(UiTextProps {
            ...
            .common(
                UiCommonProps::default()
                    .margin(UiEdgeInsets {
                        left: UiDimension::Px(64),
                        ..UiEdgeInsets::default()
                    })
                    .padding(UiEdgeInsets {
                        left: UiDimension::Px(28),
                        ..UiEdgeInsets::default()
                    }),
            )
        ...
        draw_quote(
            ...
            area(),
            palette,
            UiTreeTextMetrics::for_node(&quote),
        );
        assert!(canvas.pixels().contains(&palette.hover_background));
    }
