...
mod tests {
    ...
    #[test]
    fn measurement_covers_incremental_absolute_row_scroll_image_and_padding_paths() {
        use crate::test_assert::KucTestExpect;
        use katana_ui_core::render_model::{
            UiCommonProps, UiEdgeInsets, UiImageSurfaceProps, UiPosition,
        };
        ...
        let column =
            UiNode::new(UiNodeKind::Column, "").child(UiNode::new(UiNodeKind::Text, "row"));
        assert!(can_render_children_incrementally(&column));
        let hover = column.clone().visual_role(UiVisualRole::HoverSurface);
        assert!(!can_render_children_incrementally(&hover));

        let absolute = UiNode::new(UiNodeKind::Button, "overlay").position(UiPosition::Absolute);
        let stack = UiNode::new(UiNodeKind::Stack, "").child(absolute);
        assert!(can_render_partial_media_frame_stack(&stack));
        assert!(!can_render_children_incrementally(&stack));

        let row = UiNode::new(UiNodeKind::Row, "")
            .child(UiNode::new(UiNodeKind::Button, "left"))
            .child(UiNode::new(UiNodeKind::Toggle, "right"));
        assert_eq!(TEXT_HEIGHT, measured_node_height(&row, context, 0, area));
        let scroll = UiNode::new(UiNodeKind::ScrollArea, "");
        assert_eq!(0, measured_node_height(&scroll, context, 0, area));

        let image = UiImageSurfaceProps::new("image", 3, 5, vec![0xff; 3 * 5 * 4])
            .kuc_expect("test image must be valid");
        let image_node = UiNode::new(UiNodeKind::ImageSurface, "").image_surface(image);
        assert_eq!(5, measured_node_height(&image_node, context, 0, area));

        let padded = UiNode::new(UiNodeKind::Card, "Card")
            .common(
                UiCommonProps::default()
                    .padding(UiEdgeInsets::axis(UiDimension::Px(3), UiDimension::Px(4))),
            )
            .child(UiNode::new(UiNodeKind::Text, "body"));
        assert_eq!(3, ContainerPadding::from_node(&padded).left);
        assert_eq!(4, ContainerPadding::from_node(&padded).top);
        assert!(measured_node_height(&padded, context, 0, area) > TEXT_HEIGHT);
        assert_eq!(0, container_gap(&padded));
        assert_eq!(
            0,
