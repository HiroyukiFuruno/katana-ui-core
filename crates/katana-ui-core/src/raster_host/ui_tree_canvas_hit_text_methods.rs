...
impl UiTreeHostActionHitCollector<'_> {
    ...
    pub(super) fn span_text_renderers(&self, node: &UiNode) -> SpanTextRenderers<'_> {
...
mod tests {
    ...
    #[test]
    fn text_renderer_selects_code_and_export_font_roles() {
        ...
        let code = UiNode::new(UiNodeKind::Text, "alpha beta").font_role("code");
        ...
        assert!(std::ptr::eq(collector.text_renderer(&plain), &text));
    }
}
