...
#[cfg(test)]
mod tests {
    use super::{draw_switch, draw_switch_with_disabled, switch_thumb_fill, switch_track_fill};
    ...
    #[test]
    fn disabled_and_light_switches_keep_their_explicit_visual_tokens() {
        let dark = VisualPalette::from_theme(&ThemeSnapshot::dark());
        assert_eq!(
            super::mix_color(dark.surface, dark.muted, super::COLOR_ALPHA_MAX / 2),
            switch_track_fill(&dark, true, true)
        );
        assert_eq!(dark.muted, switch_thumb_fill(&dark, true));

        let light = VisualPalette::from_theme(&ThemeSnapshot::light());
        assert_eq!(
            super::SWITCH_THUMB_LIGHT_THEME,
            switch_thumb_fill(&light, false)
        );

        let mut canvas = Canvas::new(80, 48, dark.background);
        draw_switch_with_disabled(
            &mut canvas,
            &dark,
            SWITCH_X,
            SWITCH_Y,
        ...
        assert!(canvas.pixels().contains(&dark.muted));
    }
