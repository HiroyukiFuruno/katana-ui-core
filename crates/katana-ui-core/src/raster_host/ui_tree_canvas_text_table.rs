...
mod tests {
    ...
    #[test]
    fn table_draw_paints_header_alternating_rows_and_aligned_cells() {
        let renderer = TextRenderer::load(&UiCoreFacade::default(), "body");
        ...
        let mut canvas = Canvas::new(260, 220, palette.background);
        UiTreeTextTable::draw(
            &mut canvas,
            UiTreeTextTableContext {
                renderer: &renderer,
        ...
        assert!(canvas.pixels().contains(&palette.table_header_background));
        assert!(canvas.pixels().contains(&palette.table_background));
        assert!(canvas.pixels().contains(&palette.table_even_row_background));
        ...
        assert!(cell_text_x("b", 0, &layout, 1) > cell_text_x("b", 0, &layout, 0));
        assert!(cell_text_x("c", 0, &layout, 2) > cell_text_x("c", 0, &layout, 1));
    }
}
