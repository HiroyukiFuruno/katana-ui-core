...
mod tests {
    ...
    #[test]
    fn entry_and_select_renderers_preserve_label_value_and_selector_geometry() {
        let theme = ThemeSnapshot::dark();
        let facade = UiCoreFacade::new(theme.clone());
        let text = TextRenderer::load(&facade, "body");
        let palette = UiTreeCanvasPalette::from_theme(&theme);
        let mut canvas = Canvas::new(320, 80, palette.background);
        let input = UiNode::new(UiNodeKind::Input, "Input label")
            .placeholder("placeholder")
            .interaction(UiInteractionState {
                value: "typed value".to_string(),
                ..UiInteractionState::default()
            });
        let mut input_y = 4;

        UiTreeEntryRenderer::draw_text_entry(
            &mut canvas,
            &text,
            &input,
            8,
        ...
        assert_eq!(4 + TEXT_HEIGHT, input_y);
        assert!(
            canvas
                .text_runs()
                .iter()
                .any(|run| run.text() == "Input label")
        );
        assert!(
            canvas
                .text_runs()
                .iter()
                .any(|run| run.text() == "typed value")
        );

        let select = UiNode::new(UiNodeKind::SelectBox, "Choose").placeholder("choice");
        let mut select_y = input_y;
        UiTreeEntryRenderer::draw_select(
            &mut canvas,
            &text,
            &select,
            8,
        ...
        assert_eq!(input_y + TEXT_HEIGHT, select_y);
