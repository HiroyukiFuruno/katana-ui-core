...
mod tests {
    ...
    use super::{
        DIAGRAM_CONTROL_SPACER_Y_OFFSET, QUOTE_INDENT, UiTreeDocumentTypography, UiTreeTextMetrics,
    ...
    #[test]
    fn role_backgrounds_and_alignment_preserve_surface_contracts() {
        let theme = ThemeSnapshot::dark();
        let palette = UiTreeCanvasPalette::from_theme(&theme);
        let area = UiTreeRenderArea {
            x: 0,
        ...
        let error = text_role_node("media-error");
        let mut canvas = Canvas::new(100, 60, palette.background);
        UiTreeTextRoleRenderer::draw_background(
            &mut canvas,
            &error,
            8,
            ...
            area,
            palette,
            UiTreeTextMetrics::for_node(&error),
        );
        assert_eq!(palette.danger_accent, canvas.pixels()[4 * 100 + 8]);
        assert_eq!(palette.alert_background, canvas.pixels()[4 * 100 + 20]);

        let centered = UiNode::new(UiNodeKind::Text, "centered")
            .text(UiTextProps {
                role: "heading-html-centered".to_string(),
                ..UiTextProps::default()
            })
            .width(UiDimension::Px(80));
        let right = UiNode::new(UiNodeKind::Text, "right")
            .text(UiTextProps {
        ...
        assert_eq!(
            14,
            UiTreeTextRoleRenderer::aligned_x(&centered, 0, area, 60)
        );
        assert_eq!(24, UiTreeTextRoleRenderer::aligned_x(&right, 0, area, 60));
    }

    #[test]
    fn code_and_quote_alignment_accumulate_indent_and_explicit_text_padding() {
        let area = UiTreeRenderArea {
            x: 0,
            y: 0,
            width: 160,
        ...
        let common = |padding_left| {
            UiCommonProps::default()
                .margin(UiEdgeInsets {
                    left: UiDimension::Px((QUOTE_INDENT * 2) as u16),
                    ..UiEdgeInsets::default()
                })
                .padding(UiEdgeInsets {
                    left: UiDimension::Px(padding_left),
                    ..UiEdgeInsets::default()
                })
        };
        let code = UiNode::new(UiNodeKind::Text, "code")
            .text(UiTextProps {
                role: "code".to_string(),
                ..UiTextProps::default()
            })
            .common(common(7));
        let padded_quote = UiNode::new(UiNodeKind::Text, "quote")
            .text(UiTextProps {
                role: "blockquote".to_string(),
                ..UiTextProps::default()
            })
            .common(common(5));
        let plain_quote = UiNode::new(UiNodeKind::Text, "quote")
            .text(UiTextProps {
                role: "blockquote".to_string(),
                ..UiTextProps::default()
            })
        ...
        assert_eq!(
            (9 + QUOTE_INDENT * 2 + 7) as isize,
            UiTreeTextRoleRenderer::aligned_x(&code, 9, area, 0)
        );
        assert_eq!(
            (9 + QUOTE_INDENT * 2 + 5) as isize,
            UiTreeTextRoleRenderer::aligned_x(&padded_quote, 9, area, 0)
        );
        assert_eq!(
            (9 + QUOTE_INDENT * 2) as isize,
            UiTreeTextRoleRenderer::aligned_x(&plain_quote, 9, area, 0)
        );
    }
