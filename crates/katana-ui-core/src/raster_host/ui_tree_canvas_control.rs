...
mod tests {
    ...
    #[test]
    fn control_proxies_preserve_default_button_advance_and_entry_dispatch() {
        let theme = ThemeSnapshot::dark();
        let palette = UiTreeCanvasPalette::from_theme(&theme);
        let text = TextRenderer::load(&UiCoreFacade::new(theme), "body");
        let mut canvas = Canvas::new(320, 100, BACKGROUND);
        let mut y = 0;
        let button: UiNode = Button::new("Default").into();

        super::UiTreeControlRenderer::draw_button(&mut canvas, &text, &button, 0, &mut y, palette);
        assert_eq!(TEXT_HEIGHT + BUTTON_DEFAULT_ROW_GAP, y);

        let input = UiNode::new(UiNodeKind::Input, "Input").placeholder("value");
        super::UiTreeControlRenderer::draw_text_entry(
            &mut canvas,
            &text,
            &input,
            0,
        ...
        let select = UiNode::new(UiNodeKind::SelectBox, "Select").placeholder("choice");
        super::UiTreeControlRenderer::draw_select(
            &mut canvas,
            &text,
            &select,
            0,
