...
use crate::text_raster::{PlatformTextRaster, PlatformTextRasterizer};
use crate::text_surface::{TextSurfaceAnnotationStyle, TextSurfaceFrameRecord};
...
pub(super) fn build_paint_plan(
    ...
    for bounds in &frame.selection.rects {
        operations.push(fill(
            ...
            clip_to_viewport(*bounds, frame.viewport_bounds),
    ...
    if let Some(preedit) = frame.preedit.as_ref() {
        for bounds in &preedit.rects {
            operations.push(fill(
                ...
                frame.viewport_bounds,
                clip_to_viewport(bottom_stroke(*bounds), frame.viewport_bounds),
                style.preedit_rgba,
            ));
        }
    }
            ...
            match annotation.style {
                ...
                TextSurfaceAnnotationStyle::Outline => {
                    operations.push(fill(
                        EguiTextSurfaceDrawLayer::Annotation,
                        frame.viewport_bounds,
                        clip_to_viewport(top_stroke(*bounds), frame.viewport_bounds),
                        color,
                    ));
                    operations.push(fill(
                        EguiTextSurfaceDrawLayer::Annotation,
    ...
    if frame.selection.caret.width > 0 && frame.selection.caret.height > 0 {
        operations.push(fill(
            ...
            frame.viewport_bounds,
            clip_to_viewport(frame.selection.caret, frame.viewport_bounds),
            style.caret_rgba,
        ));
    }
