...
use crate::molecule::command_chrome::{CommandChromeToolbar, CommandChromeToolbarAction};
                ...
                for (action, rendered) in actions.iter().zip(rendered) {
                    ...
                    let primary_target_class = if action.dropdown_model().is_some_and(|dropdown| {
                        matches!(
                            ...
                            crate::molecule::command_chrome::CommandChromeDropdownTrigger::Primary
                        )
                    }) {
                        crate::egui::text_command_surface::accesskit_evidence::AccessKitTargetClass::DropdownTrigger
                    } else {
                        target_class
                    };
                    ...
                    if let Some(dropdown) = action.dropdown_model() {
                        let trigger_bounds = if matches!(
                            dropdown.trigger_model(),
                            crate::molecule::command_chrome::CommandChromeDropdownTrigger::SplitSecondary
                        ) {
                    ...
                    frames.push(EguiCommandChromeActionFrame {
                        ...
                        primary_dropdown_trigger: action.dropdown_model().is_some_and(|dropdown| matches!(dropdown.trigger_model(), crate::molecule::command_chrome::CommandChromeDropdownTrigger::Primary)),
