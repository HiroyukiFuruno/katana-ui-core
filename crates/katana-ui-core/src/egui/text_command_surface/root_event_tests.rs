    ...
    fn router_context_snapshots_all_generic_payloads_without_consuming_root_batch() {
        let search_events = vec![
            CommandChromeSearchEvent::Strip {
                event: crate::molecule::structured::SearchControlStripEvent::SearchQueryChanged(
                    String::from("needle"),
                ),
            },
            CommandChromeSearchEvent::Strip {
                event: crate::molecule::structured::SearchControlStripEvent::ReplaceValueChanged(
                    String::from("replacement"),
                ),
            },
            CommandChromeSearchEvent::Strip {
                event: crate::molecule::structured::SearchControlStripEvent::SearchOptionChanged {
    ...
    impl KucRootEventBatchDispatcher for LifecycleDispatcher {
        ...
        fn dispatch_search_events(
            ...
            assert!(events.iter().any(|event| matches!(
                event,
                CommandChromeSearchEvent::Strip {
                    event: crate::molecule::structured::SearchControlStripEvent::SearchQueryChanged(value)
                } if value == "needle⭐️"
            )));
            Ok(())
        }
