...
use crate::egui::text_command_surface::{
    SanitizedCommandGroup, SanitizedCommandItem, SanitizedCommandProjection, SanitizedCommandTarget,
};
...
fn readonly_raw_text_and_ime_do_not_mutate_but_pointer_selection_remains_available() {
    ...
    let start = egui::pos2(
        content.x as f32 + SELECTION_HORIZONTAL_INSET,
        content.y as f32 + content.height as f32 / 2.0,
    );
    let end = egui::pos2(
        content.x as f32 + content.width as f32 - SELECTION_HORIZONTAL_INSET,
        start.y,
    );
        ...
        assert!(!frame.output.evidence_text.events.iter().any(|event| {
            matches!(
                ...
                crate::text_surface::TextSurfaceEvent::TextArea(
                    crate::atom::TextAreaEvent::TextInput(_)
                        | crate::atom::TextAreaEvent::ImeComposition(_)
                        | crate::atom::TextAreaEvent::ImeCommit(_)
                        | crate::atom::TextAreaEvent::Change(_)
                )
            )
        }));
    }
...
fn readonly_does_not_assign_semantics_to_an_enabled_opaque_command() {
    ...
    let projection = SanitizedCommandProjection::new([SanitizedCommandGroup::new(1, "generic")
        .item(
            SanitizedCommandItem::new(
                SanitizedCommandTarget::from_opaque_bytes([7]).with_unit_capability({
            ...
            .with_icon(crate::render_model::UiIconProps::new("<svg/>")),
        )]);
    let mut root = SanitizedDocumentRootFactory::new()
        .retain(
