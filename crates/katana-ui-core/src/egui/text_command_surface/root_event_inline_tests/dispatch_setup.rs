...
pub(super) fn source_submission() -> SourceAddressSubmission {
    let mut strip = crate::molecule::structured::source_address_strip::SourceAddressStrip::new(
        crate::molecule::structured::source_address_strip::SourceAddressPresentation::new(
            "表示", "ツールチップ", "アクセシビリティ",
        ),
    );
    let _ = strip.apply_action(
        crate::molecule::structured::source_address_strip::SourceAddressAction::SetDraft(
            "opaque-source-draft".to_owned(),
        ),
    );
    match strip.apply_action(
        crate::molecule::structured::source_address_strip::SourceAddressAction::Submit,
    ) {
        Some(
            crate::molecule::structured::source_address_strip::SourceAddressEvent::Submitted(
                submission,
            ),
        ) => submission,
