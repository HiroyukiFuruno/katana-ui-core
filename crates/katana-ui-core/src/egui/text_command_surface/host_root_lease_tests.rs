...
fn retain_with_lease_attaches_auxiliary_projection_lease_slots() {
    ...
    assert!(
        output
            .artifact_order()
            .contains(&crate::egui::text_command_surface::EguiTextCommandSurfaceChild::StatusBar)
    );
    ...
    assert!(
        output
            .artifact_order()
            .contains(&crate::egui::text_command_surface::EguiTextCommandSurfaceChild::Preview)
    );
