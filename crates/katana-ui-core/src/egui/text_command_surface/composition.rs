...
use crate::molecule::command_chrome::FloatingCommandToolbarEvent;
use crate::render_model::UiRect;
use crate::text_surface::TextSurfaceAction;
...
impl EguiTextCommandSurfaceAdapter {
    pub(crate) fn show_with_tab_strip(
        ...
        status_bar: Option<&mut crate::molecule::StatusBar>,
        diagnostics_list: Option<&mut crate::molecule::DiagnosticsList>,
        ...
        if search.as_ref().is_some_and(|value| {
            value.events.iter().any(|event| {
                matches!(
                    event,
                    crate::molecule::command_chrome::CommandChromeSearchEvent::CloseRequested
                )
            })
        }) {
        ...
        let text = self.show_text_in(
            ...
            self.context_menu
                .as_ref()
                .is_some_and(crate::egui::context_menu::EguiContextMenuAdapter::is_open),
        )?;
