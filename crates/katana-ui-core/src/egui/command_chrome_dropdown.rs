...
pub(super) struct RenderedDropdownItem {
    item: crate::molecule::command_chrome::CommandChromeDropdownItem,
...
pub(super) fn show_dropdown(
        ...
        .show(ui.ctx(), |menu_ui| {
            ...
            for (index, item) in rendered.iter().enumerate() {
                ...
                publish_labeled_button_accesskit(
                    ...
                    row_bounds,
                    item.item.id().as_str(),
                    crate::egui::text_command_surface::accesskit_evidence::AccessKitTargetClass::DropdownItem,
                );
                let pointer_key = (
    ...
    if pointer_pressed_outside(ui, bounds, toolbar_bounds, &items) {
        events.extend(
            toolbar.apply_action(CommandChromeToolbarAction::DismissDropdown {
                reason:
