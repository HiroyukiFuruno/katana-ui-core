...
use crate::text_raster::{PlatformFontCatalog, PlatformTextMetricsFrame, PlatformTextRasterConfig};
...
fn command_chrome_adapter_rejects_a_catalog_from_a_different_font_policy() {
    ...
    assert!(matches!(
        ...
        EguiCommandChromeError::Text(
            crate::text_raster::PlatformTextRasterError::CatalogConfigurationMismatch
        )
    ));
}
