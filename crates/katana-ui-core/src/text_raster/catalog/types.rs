...
pub(crate) struct PlatformRegularFontFaces {
    pub(crate) proportional: Vec<PlatformRegularFontFace>,
    pub(crate) monospace: Vec<PlatformRegularFontFace>,
}
...
impl PlatformRegularFontFaces {
    ...
    pub(crate) fn iter(&self) -> impl Iterator<Item = &PlatformRegularFontFace> {
        self.proportional.iter().chain(&self.monospace)
    }

    pub(crate) fn first_candidates(&self) -> impl Iterator<Item = &PlatformRegularFontFace> {
        self.proportional
            .first()
            .into_iter()
            .chain(self.monospace.first())
    }
}
...
pub struct PlatformFontCatalog {
    ...
    pub(super) candidate_chain_font_system: Mutex<Option<FontSystem>>,
