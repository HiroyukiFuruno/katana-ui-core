...
fn run_frame(
    root: &mut katana_ui_core::egui::text_command_surface::SanitizedDocumentRoot,
) -> Result<
    katana_ui_core::egui::text_command_surface::SanitizedDocumentRootFrame,
    katana_ui_core::egui::text_command_surface::SanitizedDocumentRootFactoryError,
> {
...
fn invalid_public_command_icon_fails_closed_at_the_root_boundary() {
    ...
    assert!(matches!(
        error,
        katana_ui_core::egui::text_command_surface::SanitizedDocumentRootFactoryError::Render(_)
    ));
}
