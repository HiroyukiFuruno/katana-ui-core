...
fn assert_context_artifact(
    ...
    assert!(matches!(
        plans.last(),
        Some(katana_ui_core::egui::artifact_compositor::ArtifactPaintPlanRef::ContextMenu(_))
    ));
    Ok(())
}
...
fn context_menu_output(
    output: &EguiTextCommandSurfaceOutput,
) -> &katana_ui_core::egui::context_menu::EguiContextMenuOutput {
    output
        .context_menu
        .as_ref()
...
fn context_menu_record(
    output: &EguiTextCommandSurfaceOutput,
) -> &katana_ui_core::egui::context_menu::EguiContextMenuFrameRecord {
    context_menu_output(output)
        .record
        .as_ref()
