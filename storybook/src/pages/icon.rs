...
fn icon_row(
    ...
    h_stack((
        ...
        svg(svg_content).style(move |s| s.width(size_px).height(size_px).color(color)),
...
fn page_content(theme: &Theme) -> impl IntoView + use<> {
    ...
    let (ar, ag, ab) = (
        accent_resolved.color_r,
        accent_resolved.color_g,
        accent_resolved.color_b,
    );
    let (dr, dg, db) = (
        danger_resolved.color_r,
        danger_resolved.color_g,
        danger_resolved.color_b,
    );
    ...
    let (r0, g0, b0, svg0, sz0) = (
        check_rows[0].3,
        check_rows[0].4,
        check_rows[0].5,
        check_rows[0].1.clone(),
        check_rows[0].2,
    );
    let (r1, g1, b1, svg1, sz1) = (
        check_rows[1].3,
        check_rows[1].4,
        check_rows[1].5,
        check_rows[1].1.clone(),
        check_rows[1].2,
    );
    let (r2, g2, b2, svg2, sz2) = (
        check_rows[2].3,
        check_rows[2].4,
        check_rows[2].5,
        check_rows[2].1.clone(),
        check_rows[2].2,
    );
    let (r3, g3, b3, svg3, sz3) = (
        check_rows[3].3,
        check_rows[3].4,
        check_rows[3].5,
        check_rows[3].1.clone(),
        check_rows[3].2,
    );
