...
use floem::views::{h_stack, h_stack_from_iter, label, scroll, v_stack, v_stack_from_iter, Decorators,
};
use floem::IntoView;
...
fn page_content(theme: &Theme) -> impl IntoView + use<> {
    ...
    rows.push(v_stack((
        label(|| "Badge Samples")
            .style(|s| s.font_size(16.0).margin_bottom(8.0)),
    ...
    for (variant, vname) in variants {
        ...
        rows.push(
            v_stack((
                ...
                h_stack_from_iter(cells).style(|s| s.gap(4.0).flex_wrap(floem::style::FlexWrap::Wrap)),
            ))
            .style(|s| s.gap(4.0)),
        );
    }
