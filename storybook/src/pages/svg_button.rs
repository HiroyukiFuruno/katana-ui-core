...
fn page_content(theme: &Theme) -> impl IntoView + use<> {
    ...
    let loading_log = create_rw_signal("loading は click を抑止します".to_string());
    ...
    let (dr, dg, db) = (
        disabled_r.icon_color.r,
        disabled_r.icon_color.g,
        disabled_r.icon_color.b,
    );
    ...
    scroll(
        v_stack((
            label(|| "SvgButton Samples").style(|s| s.font_size(16.0).margin_bottom(8.0)),
            ...
            SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "live check")
                ...
                .view(theme.clone(), {
                    ...
                    move || click_log.set("クリック: Filled/Accent".to_string())
                }),
            label(move || format!("callback log: {}", click_log.get()))
                .style(|s| s.font_size(12.0)),
            label(|| "Variant × Tone grid")
                .style(|s| s.font_size(16.0).margin_top(12.0).margin_bottom(8.0)),
            label(|| "Size display")
                .style(|s| s.font_size(16.0).margin_top(12.0).margin_bottom(8.0)),
            h_stack((
                SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "small")
                    .size(IconSize::Sm)
                    .view(theme.clone(), || {}),
                SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "medium")
                    .size(IconSize::Md)
                    .view(theme.clone(), || {}),
                SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "large")
                    .size(IconSize::Lg)
                    .view(theme.clone(), || {}),
            ))
            ...
            label(|| "Loading state")
                .style(|s| s.font_size(16.0).margin_top(12.0).margin_bottom(8.0)),
            h_stack((
                SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "clickable")
                    .variant(Variant::Filled)
                    .tone(Tone::Accent)
                    .view(theme.clone(), {
                        let loading_log = loading_log;
                        move || loading_log.set("clickable: callback fired".to_string())
                    }),
                SvgButton::new(IconSource::SvgBytes(ICON_CHECK), "loading")
                    .variant(Variant::Filled)
                    .tone(Tone::Accent)
                    .loading(true)
                    .view(theme.clone(), {
                        let loading_log = loading_log;
                        move || loading_log.set("loading: callback fired (NG)".to_string())
                    }),
                label(move || loading_log.get()).style(|s| s.font_size(12.0)),
            ))
            .style(|s| s.gap(8.0).items_center()),
            h_stack((
                button_cell(
