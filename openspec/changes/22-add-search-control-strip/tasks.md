...
- [x] 4.1 compact / expanded / disabled / invalid regex の主要 preset を契約データで回帰する。
- [x] 4.2 match case / whole word / regex toggle の on / off を設定 mutation で回帰する。
- [x] 4.3 result count 0 / 1 / many、active index あり / なしを contract test で回帰する。
- [x] 4.4 light / dark theme を Storybook gate の theme token 差分で回帰する。
...
- [x] 5.3 settings で query、option、replace mode、result count、active index を切替えできるようにする。
- [x] 5.4 state に query、options、replace value、result summary を表示する。
- [x] 5.5 action / event log に query、option、navigation、replace を表示する。
- [x] 5.6 quality に option label、state id、result count、event contract の検証結果を表示する。

## 6. ドキュメント

- [x] 6.1 `SearchBox` は simple query input、`SearchControlStrip` は検索操作 row として docs に明記する。
- [x] 6.2 `docs/inventory/katana-katana-chat-ui-kdv-kle-ui-needs.md` の SearchControlStrip 行と同期する。
