...
`katana` の preview pane / explorer / problems panel、`katana-chat-ui` の message list / adapter bar、`katana-markdown-linter` の lint 進行中など、画面 load 中に「形状だけのプレースホルダ（skeleton）」を出したい箇所が広範に存在する。現状は Spinner / LoadingDots や空白で代用しており、layout のがたつき（layout shift）と UX 品質が低い。
