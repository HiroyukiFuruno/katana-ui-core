...
- [x] 3.2 `NotificationToast` 単体 API は維持し、Storybook ページから ToastStackManager へのリンクを足す。
...
- [x] 5.1 position 6 種類の積み方向を layout / rendering contract で検証する。
- [x] 5.2 severity 4 種類（Info/Success/Warning/Danger）の tone contract を検証する。
- [x] 5.3 action 0/1/2 個の layout と primary / secondary button variant を検証する。
- [x] 5.4 stack_gap、enter / exit 方向を numeric contract で検証する。
- [x] 5.5 light / dark theme を theme token contract で検証する。

## 6. Storybook ページ

- [x] 6.1 `Disclosure > ToastStackManager` ノードを追加する。
- [x] 6.2 preset「位置 6 種類」「dedup ById」「pause_on_hover」「queue 上限超過」「action 付き toast」を実装する。
- [x] 6.3 settings で position / max_visible / dedup / duration / pause_on_hover / stack_gap を切替えできるようにする。

## 7. ドキュメント

- [x] 7.1 `docs/architecture/ui-separation/owned-ui-task-map.md` に ToastStackManager 行を追加する。
- [x] 7.2 NotificationToast ページから ToastStackManager への参照を追記する。
...
- [x] 8.4 数値化された描画契約 / 入力回帰 / Storybook requirement gate をパスする。
