...
- [ ] 2.5 `state.rs` で active_tab_id / overflow_visible / drag_in_progress / pending_close_confirm を持つ親 state、子 tab の `UiStateId` 分離を実装する。
- [ ] 2.6 `overflow.rs` でしきい値判定を純関数として実装する。
- [ ] 2.7 `keyboard.rs` で `Cmd/Ctrl+Tab`、`Cmd/Ctrl+W`、`Cmd/Ctrl+1..9` の操作を実装する。

## 3. 依存連携

- [ ] 3.1 `02-add-drag-drop-primitive` の DragSource / DropTarget / DropIndicator / DragPreview を使う。
- [ ] 3.2 `01-add-context-menu` の `ContextMenu` を tab 右クリック / group header 右クリックで使う。
...
- [ ] 5.1 overflow しきい値判定が measured width に基づき正しく hidden tab を計算することを純関数で検証する。
- [ ] 5.2 pinned tab が左端固定であり、unpinned が pinned 領域へ移動できないことを検証する。
- [ ] 5.3 dirty tab close で `TabCloseRequested` → `ConfirmClose` action → `TabClosed` の順を検証する。
- [ ] 5.4 ドラッグ並べ替えで `TabReordered { from, to }` が正しく発火することを検証する。
- [ ] 5.5 group 内 / 間 / 新規グループ作成のドロップが正しく state に反映することを検証する。
- [ ] 5.6 キーボード `Cmd/Ctrl+1..9` で n 番目の visible tab が active になることを検証する。
- [ ] 5.7 子 tab の `UiStateId` がユニークであり、親 state と独立であることを検証する。

## 6. 画像回帰

- [ ] 6.1 default / overflow / pinned 混在 / dirty 混在の表示を回帰する。
- [ ] 6.2 ドラッグ中の ghost + drop indicator（before / after / inside-group / new-group）を回帰する。
- [ ] 6.3 group collapsed / expanded、group color tone を回帰する。
- [ ] 6.4 light / dark theme での tab tone（Default / Accent / Warning / Danger / Muted）を回帰する。
...
- [ ] 9.3 `openspec validate 03-add-workspace-tab-bar --strict` をパスする。
- [ ] 9.4 画像回帰 / 入力回帰の CI gate をパスする。
