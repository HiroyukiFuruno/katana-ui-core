...
- [ ] 1.2 `MotionSpec` / `MotionPrimitive` の typed model を確定する。
- [ ] 1.3 `ReducedMotionPolicy` 3 種類と `disable_in` context list を確定する。
- [ ] 1.4 各 molecule の default motion を確定する。

## 2. theme / interaction 実装

- [ ] 2.1 `theme/` に motion tokens を追加する。
- [ ] 2.2 `interaction/motion.rs` を新設する。
...
- [ ] 4.1 `compute_motion(reduced_motion, spec)` 純関数が Reduced で Instant を返すことを検証する。
- [ ] 4.2 各 molecule の motion default が dataset 通りであることを検証する。
- [ ] 4.3 `disable_in = [Storybook]` で Storybook context のアニメーションが Instant になることを検証する。
- [ ] 4.4 Force / Ignore モードで OS 設定を上書きできることを検証する。
- [ ] 4.5 Shimmer が reduced 時に無効化されることを検証する。
...
- [ ] 6.1 `Foundation > Motion` ノードを catalog に追加する。
...
- [ ] 8.3 `openspec validate 18-add-animation-primitives --strict` をパスする。
