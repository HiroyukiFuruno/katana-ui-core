...
| Tabs | selected tab、tabs、disabled tab、orientation、closeable、pinned、group id、dirty marker、icon | select tab、close tab、close others / all / left / right、pin / unpin、move within group、context menu、keyboard navigation | tab change、tab close、tab close request、pin change、group change、context command、dirty marker event、focus | selected index、focused tab、closed tab ids、pinned ids、group state、context menu state、dirty ids、child page state | horizontal、vertical、disabled tab、closeable、pinned、grouped、dirty、icon tabs、context menu | keyboard、state persistence、close / pin / group / dirty / icon marker、right-click context menu | preset tabs sample |
...
Tabs は単なる preset 切替だけではなく、アプリ UI の tab bar として必要な close、close others / all / left / right、pin、group、dirty、icon、right-click context menu を core 契約に含める。
