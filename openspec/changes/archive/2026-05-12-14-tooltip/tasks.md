...
- [x] 現状は未実装扱いとし、文字を並べた疑似表示を完了条件にしない。hover / focus / delay / close の実 event に反応して tooltip が表示されることを必須にする。
- [x] tooltip 本体は overlay として anchor から独立配置し、top / bottom / start / end、画面端反転、最大幅折り返し、light / dark を実 widget で確認できるようにする。
- [x] Storybook は「hover/focus する対象」と「実際に出る tooltip」を中心に組み直し、操作できない説明ラベルだけのサンプルを除去する。
