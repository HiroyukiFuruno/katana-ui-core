...
- [x] 3.2 ページ内表示
  ...
  - [x] light / dark 追従
  - [x] header/body/footer/actions/content slot を使った live サンプル
  - [x] liveサンプル内に TextInput / Button / Badge / Accordion を配置

## 4. 完了確認

- [x] 4.1 `cargo check -p katana-ui-widget`
- [x] 4.2 `just storybook` で card ページが想定通り表示
  - 2026-05-11: `KATANA_UI_WIDGET_STORYBOOK_PAGE=card` で起動し、katana方式の `screencapture -l <window_id>` でCardページのウィンドウ単体表示を確認。
- [x] 4.3 ast-lint 通過
...
- [x] 現状は未実装扱いで再確認する。Card は単なる枠ではなく、複雑な UI 自身を子要素として配置できる構成にする。
- [x] header / body / footer / actions / 任意 content slot を持ち、TextInput、Button、Badge、Accordion などの複合 widget を内部に置けることを Storybook で確認する。
- [x] interactive Card の click / hover / focus は、内部の button や input の操作を壊さない責務分離にする。
