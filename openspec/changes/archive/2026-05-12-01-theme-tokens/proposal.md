...
参照元: `../katana/crates/katana-ui/src/theme_bridge/` の役割定義（adapter 依存実装はそのまま使えないため、Adapter 向けの最小トークンに再構成する）。
...
- `Theme` 構造体で全トークンをまとめ、`Theme::default_light()` / `Theme::default_dark()` を提供。Adapter の reactive コンテキストに 1 つ流す。
...
- `widget-theme-tokens`: 意味的トークン（color / spacing / typography）と Theme 構造体、light/dark の既定値、Adapter コンテキストへの注入手順。
