...
KUC MUST 中核 crate（core crate）が `adapter`, `adapter`, `adapter`, または Katana domain crates に依存したとき失敗する checks を含める。
checks MUST local quality gate の一部にする。

#### Scenario: forbidden framework dependency is added

- **WHEN** `adapter`, `adapter`, or `adapter` を `crates/katana-ui-core` に追加する
- **THEN** 依存漏れ検査（dependency leak guard）が失敗する
- **AND** dependency は KUC active workspace から除外する

#### Scenario: forbidden domain dependency is added

- **WHEN** `katana-*` domain crate を `crates/katana-ui-core` に追加する
- **THEN** 依存漏れ検査（dependency leak guard）が失敗する
- **AND** integration は consumer-facing crate または external runtime boundary へ移す

### Requirement: Storybook and tests follow core boundaries

Storybook、compile tests、snapshot tests、smoke tests MUST 中立 core と external runtime output を分けて検証する。
Storybook の担当は中立 core の catalog 検証に限定し、external runtime output は KUC active release gate に含めない。
Storybook MUST `katana-ui-core` core model だけを使い、framework-specific UI / external runtime 経由にしてはならない。
...
### Requirement: release dry-run covers core only

Release verification MUST 中核 crate（core crate）を対象にする。
repo 外の変換層はこの repository の release gate に含めない。

#### Scenario: release check runs

- **WHEN** release verification を実行する
- **THEN** core package dry-run が走る
- **AND** workspace member は core / storybook / consumer app に限定される
