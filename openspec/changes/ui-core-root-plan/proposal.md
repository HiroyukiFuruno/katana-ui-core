...
既存の OpenSpec 変更単位（OpenSpec change）は、主に Floem 前提の画面部品（widget）抽出を扱っている。
...
- 中核 crate（core crate）を Floem 直接依存（hard dependency）から切り離し、`runtime` / `window` / `surface` / `atom` / `molecule` / `layout` / `theme` / `event` / `render_model` / `accessibility` / `adapter_contract` を持つ UI Core として定義する。
- Floem / GPUI / egui は中核（core）の依存ではなく、変換層（adapter）crate の責務として扱う。
- Storybook は `katana-ui-core` の中核（core）model だけで動かし、Floem や他 adapter 経由にはしない。
...
- **BREAKING**: 中核（core）の公開 API（public API）は Floem View / GPUI Element / egui Ui などの framework 型を返さない前提に移行する。
...
- `adapter-boundary`: Floem / GPUI / egui などの framework 実装を中核（core）から分離し、変換層（adapter）として扱う境界を定義する。
- `runtime-window-surface`: `Application`、`Window`、`Surface` を含む起動・窓・描画面の中立 API（neutral API）を定義する。
- `migration-quality-gates`: Floem / GPUI / egui / Katana domain の依存漏れを防ぐ検査、Storybook、リリース検査（release gate）、差分ずれ（drift）検出を定義する。
...
- `crates/katana-ui-core-floem`、`katana-ui-core-egui`、`katana-ui-core-gpui`、`katana-ui-core-storybook` の追加または整理が必要になる。
