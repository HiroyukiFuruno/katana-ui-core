# Tasks — storybook-page-align-center

## 1. Harness 契約

- [x] 1.1 `requirements.rs` と Storybook menu に `align-center` が存在することを guard で検査する。
- [x] 1.2 `visual/dedicated.rs::draw_page` から `align-center` 専用描画へ到達することを guard で検査する。
- [x] 1.3 `catalog/preset_labels.rs` に `align-center` 用の 4 つ以上の preset を定義し、preset ごとの preview / state 差分を自動テストで固定する。
...
- [x] 2.4 `rtk ./scripts/openspec validate storybook-page-align-center --strict` を通す。
