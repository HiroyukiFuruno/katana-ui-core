...
KUC は Adapter 専用 crate ではなく、フレームワーク非依存（framework-neutral）な UI Core である。

## 採用条件

画面部品（widget）または UI model は、次の条件をすべて満たす場合だけ KUC に入れる。

1. **Framework-neutral**: public API が Adapter View / Adapter Element / adapter Ui を返さない。
...
5. **Adapter-ready**: framework-specific UI adapter が変換できる DTO / trait 境界を持つ。
...
外部 runtime は TextArea の IME request に `preedit`、`caret`、`input_kind=Multiline` を入れて core へ返す。
...
| Chat composer | chat UI protocol と chat session に依存する | chat domain |
...
| Title bar / window chrome | OS window / external runtime 固有責務を含む | consumer crate / external runtime |
| Splash screen template | 起動 lifecycle と branding を含む | consumer crate |
| Chat root / message thread / composer | chat session / external UI protocol に依存する | chat domain |
...
- 出力 event
- 状態遷移
- external runtime で再現すべき見た目
- KUC core には入れない domain 要素

## External runtime との関係

KUC core は neutral DTO / trait を提供する。
framework-specific UI 固有の描画は KUC active workspace に置かない。
対応範囲と未対応機能は KUC active tree の release gate には含めない。
KUC 側は `docs/dependency-policy.md` と public API contract で core dependency boundary を検査する。
